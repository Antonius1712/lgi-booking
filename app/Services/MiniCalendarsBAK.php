...
class Calendar
{
    ...
    public function getHTML()
    {
        ...
        $first = ($dt->dayOfWeek + 7 - $weekstarts) % 7;
    ...
    private function getDayNames()
    {
        ...
        for ($i = $weekstarts; $i < $weekstarts + 7; $i++) {
            // Sunday is Day 0
            // $day_name = utf8_strftime(
            //     $strftime_format['dayname_cal'],
            //     strtotime("next sunday + $i days")
            // );

            $dt = Carbon::now()->startOfWeek(Carbon::SUNDAY)->addDays($i);
            $day_name = $dt->translatedFormat(
                $strftime_format['dayname_cal'] = 'D'
            );
...
class MiniCalendars
{
    public static function show($year, $month, $day, $area, $room, $dmy)
    {
